/**
 * Example: Using ckc-token with raw fetch API
 *
 * This example demonstrates how to make direct HTTP requests to the ckc-token API
 * using the native fetch API without any additional libraries.
 */

export {};

// ckc-token API endpoint
const CKC_TOKEN_API_URL = 'https://t.ckcloudai.com/v1/chat/completions';
...
console.log('=== ckc-token Raw Fetch Example ===\n');
console.log('Request:');
...
try {
  ...
  const response = await fetch(CKC_TOKEN_API_URL, {
    ...
    headers: {
      ...
      'X-Title': 'ckc-token Fetch Example',
    },
